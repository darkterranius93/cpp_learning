...
p = &person; // Помещаем в указатель `p` адрес структурной переменной `person`
p->balance // Обращение к полю структуры через указатель
```
...
```cpp
union utype {
    utype(short int i); // конструктор
    ~utype(); // деструктор
    short int i;
    char ch[2];
}
```
Здесь объявляется объединение, в котором значение типа *short int* и значение типа *char* разделяют одну и ту же область памяти.

**Общий формат объявления объединения**
```cpp
union имя_объединения {
    имя_объединения(список_аргументов); // конструктор
    ~имя_объединения(); // деструктор
...
class имя_класса {
    ...
    public:
    имя_класса(); // конструктор
    имя_класса(список_аргументов); // параметризированный конструктор
    ~имя_класса(); // деструктор
    открытые данные и функции
}
```

**Общий формат передачи аргументов конструкторам**
```cpp
тип_класса имя_переменной(список_аргументов);
```

**Передача конструктору единственного аргумента**
```cpp
тип_класса(тип_аргумента аргумент); // конструктор
тип_класса объект = аргумент; // инициализация объекта 
```

# Inline (встраиваемые) фукнции
**Встраиваемая функция** - это небольшая (по объему кода) функция, код которой подставляется вместо ее вызова.
Примеры ситуаций, когда функция не может быть встроена:
* Некоторые компиляторы не генерируют встраиваемый код, если функция содержит цикл, конструкцию *switch* или конструкцию *goto*
...
**Общий формат объявления встраиваемой функции**
```cpp
inline название_функции(список_аргументов) {};
```

**Встраиваемая функция в классах**
```cpp

class название_класса {
    возвращаемый_тип название_функции(список_аргументов); // встраиваемая функция
    возвращаемый_тип название_функции(список_аргументов) { ... } // автоматически встраиваемая функция
}

inline возвращаемый_тип название_класса::название_функции(список_аргументов) {}
...
class название_класса {
    public:
    friend возвращаемый_тип название_функции(список_аргументов);
}
```
...
class example {
    ...
    public:
    friend void example_func(example example_object);
}

void example_func(example example_object) {
    int t = example_object.value // Доступ к private-члену класса
}
```

# Конструктор копии
Конструктор копии вызывается, когда один объект инициализирует другой. И при этом НЕ вызывается при операции присваивания.
...
**Пример**
При выполнении следующих инструкций будет вызван конструктор копии класса `myclass`. В первых двух случаях конструктору копии будет передана ссылка на объект `у`, а в третьем — ссылка на объект, возвращаемый функцией `func2()`.
```cpp
myclass x = y; // Объект `у` явно инициализирует объект `x`
x.func1(y); // Объект `у` передается в качестве аргумента.
y = func2(); // Объект `у` принимает объект, возвращаемый функцией.
